...
| Year | Venue | Paper Title | Repository | Note |
|:----:|:-----:| ----------- |:----------:|:----:|
|2026|`arXiv`|[SA-LIVO: Efficient LiDAR-Inertial-Visual Odometry with Subspace-Aware Degeneracy Handling](https://arxiv.org/pdf/2606.25699)|---|紧耦合 LIVO。对雷达+视觉联合信息矩阵做特征分解，按各方向信息幅度做单阈值线性钳位软门控（雷达已约束好的方向不灌视觉，雷达缺约束而视觉能补的方向满权通过），并在同一线性化点用单环 InEKF 联合更新；光度雅可比循环前算一次后复用。|
|2026|`ICRA`|[UCA-SLAM: Tightly Coupled Visual-LiDAR SLAM with DoF-wise Uncertainty-driven Constraint Analysis](https://rasevents.org/uploads/documents/pdfviewer/02/d6/222440-1341.pdf)|[![Github stars](https://img.shields.io/github/stars/NKU16y/UCA_SLAM_open.svg)](https://github.com/NKU16y/UCA_SLAM_open)|紧耦合单目视觉-雷达 SLAM。给视觉地图点和雷达体素平面做闭式不确定性传播，再量化每个特征对 6 自由度的约束强度：前端雷达残差按测量不确定性加权，视觉残差按雷达弱方向上的信息比补约束；局部 BA 用特征+位姿不确定性加权，回环用点-面 ICP 把视觉点与雷达面一起进位姿图。|
|2026|`RAL`|[A Robust Approach for LiDAR-Inertial Odometry Without Sensor-Specific Modeling](https://arxiv.org/pdf/2509.06593)|[![Github stars](https://img.shields.io/github/stars/PRBonn/rko_lio.svg)](https://github.com/PRBonn/rko_lio)|无需传感器特定噪声/扫描模型的 LIO：相邻雷达帧间对 IMU 取平均，用恒加速度+恒角速度做去畸变与 ICP 初值；在 KISS-ICP 式 scan-to-map 点对点配准上加入姿态正则，并用加速度模长标准差自适应减弱正则。用户只需外参，同一套参数可跑汽车、背包、四足、无人机。|
|2026|`IJRR`|[DCReg: Decoupled Characterization for Efficient Degenerate LiDAR Registration](https://arxiv.org/pdf/2509.06285)|[![Github stars](https://img.shields.io/github/stars/JokerJohn/DCReg.svg)](https://github.com/JokerJohn/DCReg)|面向退化雷达配准的检测–刻画–抑制：用 Schur 补把 Hessian 解耦成旋转/平移子空间再算相对条件数；把特征向量对齐到物理运动轴；只在预条件子里对弱方向做特征值钳位，用 PCG 求解且不改原最小二乘。|
|2026|`IJRR`|[A flexible framework for accurate LiDAR odometry, map manipulation, and localization](https://journals.sagepub.com/doi/pdf/10.1177/02783649251316881?casa_token=a-H5sxHrbEMAAAAA:-hiGWp-IMMVimS9zjHW0daIrG6_I86pNkdfP1NciXg1pWhLIQKeIlzY7NEDxSVLhq27oO-fdwE_P)|[![Github stars](https://img.shields.io/github/stars/MOLAorg/mola.svg)](https://github.com/MOLAorg/mola) |支持体素地图、2D/3D 占用网格、特征检测等;改善点云去畸变;回环检测算法;基于GTSAM的LO（ICP对齐）+GNSS|
|2026|`AIS`|[Voxel‐SLAM: A Complete, Accurate, and Versatile Light Detection and Ranging‐Inertial Simultaneous Localization and Mapping System](https://advanced.onlinelibrary.wiley.com/doi/pdf/10.1002/aisy.202501081)|[![Github stars](https://img.shields.io/github/stars/hku-mars/Voxel-SLAM.svg)](https://github.com/hku-mars/Voxel-SLAM)|一个完成的LiDAR SLAM系统，包括初始化、前端里程计、局部建图、回环检测（用的BTC）、全局建图，其中，前端里程计用的是lidar-IMU BA（基于滑窗的联合优化）而非滤波的方法。|
|2024|`TRO`|[Btc: A binary and triangle combined descriptor for 3-d place recognition](https://ieeexplore.ieee.org/abstract/document/10388464/)|[![Github stars](https://img.shields.io/github/stars/hku-mars/btc_descriptor.svg)](https://github.com/hku-mars/btc_descriptor)|用雷达点云做回环检测|
|2026|`IJRR`|[Memory-efficient boundary map for large-scale occupancy grid mapping](https://journals.sagepub.com/doi/pdf/10.1177/02783649261425266?casa_token=ASK-ArnH5QwAAAAA:DtULTebrENNZfs90iMpFIELymVr7VWTabRktkJGfDcZT7WiekxCoiTImsrU3JT0l3TMnwh3cZMN6)|[![Github stars](https://img.shields.io/github/stars/hku-mars/BDM.svg)](https://github.com/hku-mars/BDM)|提出The boundary map，即不再显式维护 3D 空间中所有的空闲或占据体素，只存储障碍物表面/边界处的voxel（2D边界体素）；由于地图里只有边界，作者设计了一套算法，通过沿着 $x$、$y$ 或 $z$ 轴的某个方向寻找距离目标位置最近的边界体素，就能反推出 3D 空间中任意坐标是空闲、未知还是被占据，而为了保证查询速度，作者还提出基于哈希表的 2D 网格地图作为数据结构|
...
|2025|`arXiv`|[pySLAM: An open-source, modular, and extensible framework for SLAM](https://arxiv.org/pdf/2502.11955))|[![Github stars](https://img.shields.io/github/stars/luigifreda/pyslam.svg)](https://github.com/luigifreda/pyslam)|---|
